...
type NPSProviderModel struct {
	...
	DeletePackageExecutionRules types.Bool   `tfsdk:"delete_package_execution_rules"`
}

type NPSProviderResourceData struct {
	Client                      apipb.WorkshopServiceClient
	DeletePackageExecutionRules bool
}

func configuredDeletePackageExecutionRules(value types.Bool) bool {
...
func (p *NPSProvider) Schema(ctx context.Context, req provider.SchemaRequest, resp *provider.SchemaResponse) {
	resp.Schema = schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"delete_package_execution_rules": schema.BoolAttribute{
				MarkdownDescription: "Whether deleting an `nps_workshop_package_rule` also deletes the execution rules generated by that package rule. Defaults to `false`.",
				Optional:            true,
			},
		},
	}
...
func (p *NPSProvider) Configure(ctx context.Context, req provider.ConfigureRequest, resp *provider.ConfigureResponse) {
	...
	providerData := &NPSProviderResourceData{
		Client:                      client,
		DeletePackageExecutionRules: configuredDeletePackageExecutionRules(data.DeletePackageExecutionRules),
	}
